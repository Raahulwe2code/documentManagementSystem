    ...
    connection.query("SELECT * FROM user WHERE email = '"+u_email+"'",
      (err, rows) => {
        if (err) {
          ...
          console.log(rows)
          if(rows!=""){
            res.status(200).send({"response":"email already exists, please use logIn way"})
          }else{
            console.log("false")
      ...
      if (err) {
        ...
        if(rows!=""){
          ...
          jwt.sign({ id: rows[0].id }, process.env.USER_JWT_SECRET_KEY, function(err,token){
            ...
            res.send({"response":"successfully login","token":token,"redirect_url":"http://localhost:3000/"})
          })
        }else{
